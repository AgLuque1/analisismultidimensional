...
"""
Function that creates a preprocessed pyspark dataframe that has the correct shape to use in algorithms. It build an automatic label mapping in case of classification.

Parameters:

variables_numeric: List of names of the selected numerical variables
variables_categoric: List of names of the selected categorical variables
target_variable: Name of the variable that will act as a target in the algorithm
original_df: Original pandas df with all data
problem_type: String that establish the type of problems, regression or classification

Return:

df: Preprocessed pyspark dataframe that contains the original variables and the generated labels and features
"""
def spark_df_preprocessing(variables_numeric, variables_categoric, target_variable, original_df, problem_type):
